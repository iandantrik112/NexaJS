...
export class FromHelper {
  ...
  static validateForm(processed, formValues) {
    ...
    Object.keys(processed.form).forEach(key => {
      ...
      // Skip validasi untuk field id dan field approval
      if (fieldKey === 'id' || field.type === 'approval') {
        return;
      }
  ...
  static async processFormData(formValues, processed) {
    ...
    for (const key of Object.keys(processedData)) {
      ...
      if (field.type === 'file' && value) {
        ...
        // ✅ FIX: Untuk React Native, simpan file object langsung (uri/base64) tanpa konversi ke binary array
        // FormData akan handle file upload secara otomatis
        if (typeof fileObj === 'object' && (fileObj.uri || fileObj.base64 || fileObj.name)) {
          // ✅ FIX: Simpan file object langsung untuk FormData (tidak perlu konversi ke binary array)
          processedData[key] = {
            name: fileObj.name || 'file',
            size: fileObj.size || 0,
            type: fileObj.type || 'application/octet-stream',
            uri: fileObj.uri || null,
            base64: fileObj.base64 || null
          };
        }
      }
      ...
      try {
        ...
        if (isMedia && (fieldAccept.includes('.jpg') || fieldAccept.includes('.jpeg') || fieldAccept.includes('.png'))) {
          ...
          const acceptTypes = fieldAccept.split(',').map(ext => {
            ...
            if (extClean === 'docx') return 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';
            if (extClean === 'doc') return 'application/msword';
            if (extClean === 'xls') return 'application/vnd.ms-excel';
            if (extClean === 'xlsx') return 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
            if (extClean === 'csv') return 'text/csv';
            // Default fallback
          ...
          if (isImageOnly && acceptTypes.length > 0) {
            ...
            // Pastikan semua MIME types termasuk dalam array untuk pickDocument
            // React Native Document Picker memerlukan array MIME types yang lengkap
            const documentTypes = acceptTypes.length > 0 ? acceptTypes : ['*/*'];
        ...
        if (result) {
          // ✅ FIX: Hapus validasi ukuran file - file besar diperbolehkan
          // Server akan menangani validasi ukuran file jika diperlukan
          // Tidak ada pembatasan ukuran file di client-side untuk React Native
  ...
  static buildForm(processed, formValues, setFormValues, formErrors = {}, isUpdate = false, pkgToken = null) {
    ...
    column.forEach(colName => {
      ...
      if (field) {
        ...
        // Insert mode (isUpdate = false): jangan tampilkan field id dan approval
        // Update mode (isUpdate = true): jangan tampilkan field id, field yang hidden, dan approval
        if (!isUpdate) {
