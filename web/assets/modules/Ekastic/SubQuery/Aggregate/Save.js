export async function SaveCustom(id, data, tabel,custom) {
  try {
    // Ambil data lama dari storage
    const dataform = await NXUI.ref.get(tabel.store, tabel.id);
    const oldOrder = dataform[custom]?.aggregateType || []; // perbaiki typo 'oredrBy'
    ...
    console.error("❌ SaveCustom gagal:", error);
  }
}
