...
async function initManualMode() {
  try {
    ...
    console.log("Available stores:", db.getInfo().availableStores);
...
async function addNewStores() {
  try {
    ...
    // Tambah stores baru
    await db.addStores(["products", "categories"]);

    console.log("✅ New stores added");
    console.log("Updated stores:", db.getInfo().availableStores);
...
async function createUpdateData() {
  const db = await initAutoMode();

  try {
    // Simpan data user
    const userId = await db.set("users", {
      id: "user_001",
      name: "John Doe",
...
async function readData() {
  ...
  try {
    // Get single record
    const user = await db.get("users", "user_001");
    ...
    const hasUsersStore = db.hasStore("users");
...
async function deleteData() {
  ...
  try {
    ...
    const result = await db.delete("users", "user_001");
    console.log("🗑️ Delete result:", result);

    // Verify deletion
    const deletedUser = await db.get("users", "user_001");
...
async function getLatestData() {
  ...
  try {
    ...
    const latestPost = await db.getLatest("posts", 1);
    console.log("📰 Latest post:", latestPost.data);

    // Get latest 2 records
    const latest2Posts = await db.getLatest("posts", 2);
...
async function getOldestData() {
  ...
  try {
    ...
    const oldestPost = await db.getOldest("posts", 1);
    console.log("📜 Oldest post:", oldestPost.data);

    // Get oldest 2 records
    const oldest2Posts = await db.getOldest("posts", 2);
...
async function getByDateRange() {
  ...
  try {
    ...
    const postsInRange = await db.getByDateRange("posts", startDate, endDate);
...
async function searchData() {
  ...
  try {
    ...
    const searchResults = await db.search("posts", "latest");
    console.log("🔍 Search results (all fields):", searchResults);

    // Search in specific fields
    const titleSearch = await db.search("posts", "first", ["title"]);
...
async function watchDataChanges() {
  ...
  try {
    ...
    const unwatch = db.watch("users", (event) => {
    ...
    // Test perubahan data
    await db.set("users", { id: "user_002", name: "Jane Doe" });
    await db.set("users", { id: "user_002", name: "Jane Smith" }); // Update
    await db.delete("users", "user_002"); // Delete
...
async function setupAutoRefresh() {
  ...
  try {
    ...
    const pollId = await db.setupAutoRefresh("products", {
      strategy: "polling",
      endpoint: async () => {
        // Simulasi API call
...
async function manualRefresh() {
  ...
  try {
    ...
    await db.refreshTable("products", async () => {
...
async function simpleRefresh() {
  ...
  try {
    ...
    // Setup sample data
    await db.set("users", { id: "user_1", name: "John", status: "active" });
    await db.set("products", { id: "prod_1", name: "Laptop", stock: 10 });
    await db.set("orders", { id: "order_1", total: 1000, status: "pending" });
    ...
    await db.refresh("https://api.example.com/data", {
      onProgress: (progress) => {
    ...
    await db.refreshTable("users", "https://api.example.com/users");
    console.log("🔄 Users table direfresh dari API!");

    // REFRESH MANUAL TANPA API
    await db.refreshTable("products");
...
async function batchOperations() {
  ...
  try {
    ...
    await db.batchUpdate("products", batchData, {
...
async function monitorDatabaseSize() {
  ...
  try {
    ...
    const dataSize = await db.getDataSize("users", "user_001");
...
async function startSizeMonitoring() {
  ...
  try {
    ...
    const stopMonitoring = db.monitorSize(30000);
...
async function cleanupBySize() {
  ...
  try {
    ...
    const cleanupResult = await db.cleanupBySize("logs", 10);
...
async function exportData() {
  ...
  try {
    ...
    const specificExport = await db.export(["users", "products"], {
      encrypt: true,
    ...
    await db.exportToFile("my_backup.json", ["users"], {
      encrypt: false,
      includeMetadata: true,
    });
...
async function importData() {
  ...
  try {
    ...
    // Import data
    const importResult = await db.import(importData, {
      decrypt: false,
      overwrite: true,
      onProgress: (progress) => {
        console.log(
...
async function importFromFile() {
  ...
  try {
    ...
    fileInput.onchange = async (event) => {
      ...
      if (file) {
        try {
          const importResult = await db.importFromFile(file, {
            decrypt: false,
            overwrite: true,
          });
...
async function getDatabaseInfo() {
  ...
  try {
    ...
    const realtimeStatus = db.getRealtimeStatus();
    console.log("📡 Real-time status:", realtimeStatus);

    // Check if store exists
    const hasUsers = db.hasStore("users");
...
async function userManagementExample() {
  ...
  try {
    ...
    // 3. Search users
    const adminUsers = await db.search("users", "admin", ["role", "name"]);
    console.log("🔍 Admin users found:", adminUsers.found);

    // 4. Get latest user
    const latestUser = await db.getLatest("users", 1);
    ...
    await db.exportToFile("users_backup.json", ["users"]);
    console.log("💾 Users exported to backup file");

    // 7. Monitor size
    const userStoreSize = await db.getStoreSize("users");
...
async function ecommerceExample() {
  ...
  try {
    ...
    await db.batchUpdate("products", products, { clearFirst: true });
    ...
    await db.batchUpdate("categories", categories, { clearFirst: true });
    console.log("✅ Categories initialized");

    // 3. Search products
    const laptops = await db.search("products", "laptop", ["name", "category"]);
    ...
    const unwatchProducts = db.watch("products", (event) => {
    ...
    await db.exportToFile("catalog_backup.json", ["products", "categories"]);
...
async function errorHandlingExamples() {
  try {
    ...
    let db;
    try {
      db = await dDB.indexedDB.init("MyAppDB", 1);
    } catch (error) {
    ...
    try {
      const user = await db.get("users", "nonexistent_user");
...
async function performanceOptimization() {
  ...
  try {
    ...
    await db.batchUpdate("large_items", largeDataset, {
...
async function updateMultipleFields() {
  ...
  try {
    ...
    await db.updateFields("users", "user_001", {
    ...
    // Verify update
    const updatedUser = await db.get("users", "user_001");
...
async function updateSingleField() {
  ...
  try {
    ...
    await db.updateField("users", "user_001", "age", 32);
    console.log("✅ Age updated");

    await db.updateField("users", "user_001", "role", "super_admin");
...
async function updateNestedFields() {
  ...
  try {
    ...
    await db.updateNestedField("users", "user_001", "settings.theme", "dark");
...
async function mergeDataExample() {
  ...
  try {
    ...
    await db.mergeData("users", "user_001", {
      status: "active",
      lastLogin: new Date().toISOString(),
      loginCount: 1,
    });
    ...
    const user1 = await db.get("users", "user_001");
    const user2 = await db.get("users", "user_002");
...
async function batchUpdateFieldsExample() {
  ...
  try {
    ...
    const result = await db.batchUpdateFields("users", batchUpdates, {
      onProgress: (progress) => {
        console.log(`📊 Batch progress: ${progress.percentage}%`);
      },
    });
    ...
    for (const update of batchUpdates) {
      const user = await db.get("users", update.id);
...
async function ecommercePartialUpdateExample() {
  ...
  try {
    ...
    await db.updateField("products", "prod_001", "stock", 8);
    console.log("📦 Stock updated after purchase");

    // Scenario 2: Price discount
    await db.updateField("products", "prod_001", "price", 13500000);
    console.log("💰 Price updated (discount applied)");

    // Scenario 3: Update specs
    await db.updateNestedField("products", "prod_001", "specs.ram", "32GB");
    await db.updateNestedField(
      "products",
      "prod_001",
      "specs.storage",
    ...
    await db.batchUpdateFields("products", productUpdates);
    console.log("🎯 Product featured and discount applied");

    // Final product state
    const finalProduct = await db.get("products", "prod_001");
...
async function userProfilePartialUpdateExample() {
  ...
  try {
    ...
    await db.updateFields("profiles", "profile_001", {
    ...
    // Final profile state
    const finalProfile = await db.get("profiles", "profile_001");
...
async function multipleStoresExample() {
  try {
    ...
    await db.set("users", { id: "user_1", name: "John Doe", role: "admin" });
    await db.set("products", { id: "prod_1", name: "Laptop", price: 1000 });
    await db.set("orders", {
      id: "order_1",
      userId: "user_1",
      productId: "prod_1",
    ...
    await db.set("categories", { id: "cat_1", name: "Electronics" });
    await db.set("reviews", {
      id: "rev_1",
      productId: "prod_1",
      rating: 5,
      comment: "Great!",
    });
    await db.set("settings", { id: "theme", value: "dark" });
    await db.set("logs", {
      id: "log_1",
      action: "user_login",
      timestamp: new Date(),
    ...
    const product = await db.get("products", "prod_1");
    const order = await db.get("orders", "order_1");
...
async function dynamicStoreCreation() {
  try {
    ...
    const db = await dDB.indexedDB.init("DynamicDB", 1); // Tanpa parameter stores

    // Buat tabel baru secara dinamis saat runtime
    await db.set("customers", {
      id: "cust_1",
      name: "Alice",
      email: "alice@example.com",
    });
    await db.set("suppliers", {
      id: "supp_1",
      name: "Tech Corp",
      contact: "tech@corp.com",
    });
    await db.set("inventory", { id: "inv_1", item: "Mouse", quantity: 100 });
    await db.set("transactions", { id: "trans_1", type: "sale", amount: 50 });

    console.log("✅ Dynamic stores created automatically");
    console.log("New stores:", db.listStores());
    ...
    console.log("Updated stores:", db.listStores());
...
async function multipleDatabases() {
  try {
    ...
    const mainDB = await dDB.indexedDB.init("MainAppDB", 1, [
      "users",
      "posts",
      "comments",
    ]);
    ...
    const ecommerceDB = await dDB.indexedDB.init("EcommerceDB", 1, [
      "products",
      "orders",
      "payments",
      "shipping",
    ...
    const analyticsDB = await dDB.indexedDB.init("AnalyticsDB", 1, [
      "pageviews",
      "events",
      "conversions",
      "reports",
    ...
    const cacheDB = await dDB.indexedDB.init("CacheDB", 1, [
      "api_cache",
      "image_cache",
      "user_sessions",
    ]);
    ...
    await mainDB.set("users", { id: "user_1", name: "John" });
    await ecommerceDB.set("products", { id: "prod_1", name: "Laptop" });
    await analyticsDB.set("pageviews", {
      id: "pv_1",
      page: "/home",
      views: 100,
    });
    await cacheDB.set("api_cache", {
      id: "cache_1",
      endpoint: "/api/users",
      data: [],
    });
    ...
    return { mainDB, ecommerceDB, analyticsDB, cacheDB };
  } catch (error) {
...
async function crossTableOperations() {
  try {
    ...
    const db = await dDB.indexedDB.init("CrossTableDB", 1);
    ...
    await db.set("categories", { id: "cat_1", name: "Electronics" });
    await db.set("categories", { id: "cat_2", name: "Accessories" });
...
async function storeManagementUtilities() {
  try {
    ...
    const db = await dDB.indexedDB.init("ManagementDB", 1);

    // Buat beberapa stores dengan data
    await db.set("table_a", { id: "a1", data: "Data A1" });
    await db.set("table_b", { id: "b1", data: "Data B1" });
    await db.set("table_c", { id: "c1", data: "Data C1" });

    // List semua stores
    const allStores = db.listStores();
    console.log("📋 All stores:", allStores);

    // Check store existence
    console.log("🔍 table_a exists:", db.hasStore("table_a"));
    console.log("🔍 table_d exists:", db.hasStore("table_d"));

    // Get store sizes
    for (const storeName of allStores) {
      const storeSize = await db.getStoreSize(storeName);
      console.log(`📊 ${storeName} size:`, `${storeSize.sizeKB} KB`);
    }

    // Export specific stores
    const exportResult = await db.export(["table_a", "table_b"], {
      includeMetadata: true,
    });
...
async function realWorldMultiTableExample() {
  try {
    ...
    const db = await dDB.indexedDB.init("ELearningDB", 1);
    ...
    // 3. Enrollments table
    await db.batchUpdate("enrollments", [
      {
        id: "enroll_1",
        userId: "user_1",
        courseId: "course_1",
    ...
    console.log("📚 Available tables:", db.listStores());
 ...
 * await db.set("new_table", { id: "1", data: "value" }); // Table dibuat otomatis
 *
 * // Multiple databases
 ...
 *    - Contoh: await db.refresh() // Refresh semua
 *    - Contoh: await db.refresh("https://api.com/data") // Dari API
 *
 * 2. refreshTable(storeName, apiEndpoint)
 *    - Refresh satu tabel saja
 *    - Contoh: await db.refreshTable("users") // Manual refresh
 *    - Contoh: await db.refreshTable("users", "https://api.com/users") // Dari API
 *
 * 3. stopRefresh()
 *    - Stop semua auto refresh yang berjalan
 *    - Contoh: await db.stopRefresh()
 *
 ...
 *    - Contoh: await db.updateFields("users", "user_001", { email: "new@email.com", age: 32 })
 *
 * 2. updateField(storeName, id, fieldName, fieldValue)
 *    - Update single field
 *    - Contoh: await db.updateField("users", "user_001", "email", "new@email.com")
 *
 * 3. updateNestedField(storeName, id, fieldPath, newValue)
 *    - Update nested field dengan dot notation
 *    - Contoh: await db.updateNestedField("users", "user_001", "profile.bio", "New Bio")
 *
 * 4. mergeData(storeName, id, mergeObject, options)
 *    - Merge object dengan shallow atau deep merge
 *    - Contoh: await db.mergeData("users", "user_001", { status: "active" }, { deepMerge: true })
 *
 * 5. batchUpdateFields(storeName, updates, options)
 *    - Batch update multiple records
 *    - Contoh: await db.batchUpdateFields("users", [{ id: "user_1", fields: { status: "active" } }])
 *
 * 🎯 KEUNTUNGAN:
 ...
 * // Auto Mode
 * const db = await dDB.indexedDB.init("MyAppDB", 1);
 * await db.updateField("users", "user_001", "email", "new@email.com");
 *
 * // Manual Mode
 * const db = await dDB.indexedDB.init("MyAppDB", 1, ["users"]);
 * await db.updateFields("users", "user_001", { email: "new@email.com", age: 32 });
 */
...
async function basicDuplicateExample() {
  ...
  try {
    ...
    // Basic duplication
    const duplicateResult = await db.duplicate(
      "users",
      "user_001",
      "user_001_copy"
    );
    ...
    const original = await db.get("users", "user_001");
    const duplicate = await db.get("users", "user_001_copy");
...
async function duplicateWithModificationsExample() {
  ...
  try {
    ...
    for (const size of sizes) {
      const result = await db.duplicate(
        "products",
        "product_001",
...
async function duplicateWithPreservedTimestamps() {
  ...
  try {
    ...
    const archiveResult = await db.duplicate(
      "posts",
      "post_001",
      "post_001_archive",
      {
    ...
    const copyResult = await db.duplicate(
      "posts",
      "post_001",
      "post_001_copy",
      {
    ...
    // Compare timestamps
    const original = await db.get("posts", "post_001");
    const archive = await db.get("posts", "post_001_archive");
    const copy = await db.get("posts", "post_001_copy");
...
async function duplicateWithOverwriteExample() {
  ...
  try {
    ...
    // Create first backup
    await db.duplicate("settings", "theme_config", "theme_backup", {
      modifications: {
    ...
    await db.updateFields("settings", "theme_config", {
      theme: "light",
      primaryColor: "#28a745",
      version: "1.1.0",
    });
    ...
    const overwriteResult = await db.duplicate(
      "settings",
      "theme_config",
      "theme_backup",
      {
    ...
    // Verify final state
    const currentSettings = await db.get("settings", "theme_config");
    const backup = await db.get("settings", "theme_backup");
...
async function batchDuplicateExample() {
  ...
  try {
    ...
    const backupResults = [];
    for (const user of users) {
      const result = await db.duplicate("users", user.id, `${user.id}_backup`, {
        modifications: {
...
async function userTemplatesExample() {
  ...
  try {
    ...
    for (const user of existingUsers) {
      const templateResult = await db.duplicate(
        "users",
        user.id,
        `template_${user.role}`,
        {
    ...
    const newAdminResult = await db.duplicate(
      "users",
      "template_admin",
      "new_admin_001",
      {
 ...
 *
 * // Basic duplicate
 * await db.duplicate("users", "user_001", "user_001_copy");
 *
 * // Duplicate dengan modifikasi
 * await db.duplicate("users", "user_001", "user_backup", {
 *   modifications: {
 ...
 * await db.duplicate("posts", "post_001", "post_archive", {
 *   preserveTimestamps: true,
 *   modifications: { status: "archived" }
 * });
 *
 * // Duplicate dengan overwrite
 * await db.duplicate("settings", "config", "config_backup", {
 *   overwrite: true,
...
async function addNewTablesToExistingDB() {
  try {
    ...
    console.log("📋 Tabel awal:", db.getInfo().availableStores);
    ...
    console.log("📋 Semua tabel sekarang:", db.getInfo().availableStores);
    ...
    await db.set("settings", { id: "theme", key: "theme", value: "dark" });
    await db.set("notifications", {
      id: "notif_1",
      message: "Welcome!",
      read: false,
    ...
    // 4. Verifikasi data
    const category = await db.get("categories", "cat_1");
    const review = await db.get("reviews", "rev_1");
...
async function addTablesOneByOne() {
  try {
    ...
    const db = await dDB.indexedDB.init("StepByStepDB", 1, [
      "users",
      "products",
      "orders",
    ]);

    console.log("📋 Tabel awal:", db.listStores());

    // Tambah tabel satu per satu
    console.log("➕ Menambah tabel 'categories'...");
    await db.addStores("categories");
    console.log("📋 Sekarang ada:", db.listStores());

    console.log("➕ Menambah tabel 'reviews'...");
    await db.addStores("reviews");
    console.log("📋 Sekarang ada:", db.listStores());

    console.log("➕ Menambah tabel 'inventory'...");
    await db.addStores("inventory");
    console.log("📋 Sekarang ada:", db.listStores());

    // Test dengan data
    await db.set("categories", { id: "cat_1", name: "Electronics" });
    await db.set("reviews", { id: "rev_1", rating: 5 });
    await db.set("inventory", { id: "inv_1", stock: 100 });
...
async function autoTableCreation() {
  try {
    ...
    const db = await dDB.indexedDB.init("AutoDB", 1);

    console.log("📋 Tabel awal (kosong):", db.listStores());

    // Tabel akan dibuat otomatis saat pertama kali digunakan
    await db.set("users", { id: "user_1", name: "John Doe" });
    console.log("📋 Setelah buat 'users':", db.listStores());

    await db.set("products", { id: "prod_1", name: "Laptop" });
    console.log("📋 Setelah buat 'products':", db.listStores());

    await db.set("orders", { id: "order_1", total: 1000 });
    console.log("📋 Setelah buat 'orders':", db.listStores());

    // Tambah tabel baru kapan saja
    await db.set("categories", { id: "cat_1", name: "Electronics" });
    await db.set("reviews", { id: "rev_1", rating: 5 });
    await db.set("settings", { id: "set_1", theme: "dark" });
    await db.set("analytics", { id: "ana_1", pageViews: 1000 });

    console.log("📋 Semua tabel (auto-created):", db.listStores());
...
async function addTablesWithValidation() {
  try {
    ...
    const db = await dDB.indexedDB.init("ValidatedDB", 1, [
      "users",
      "products",
    ]);
    ...
    console.log("📋 Tabel akhir:", db.listStores());
    console.log("📊 Total tabel:", db.listStores().length);
...
async function batchAddTables() {
  try {
    ...
    const db = await dDB.indexedDB.init("BatchDB", 1, ["users", "products"]);
    ...
    await db.addStores(newTables);

    console.log("✅ Batch add tabel berhasil!");
    console.log("📋 Semua tabel:", db.listStores());
    ...
    // Verifikasi data
    const tableInfo = {};
    for (const tableName of db.listStores()) {
      const allData = await db.getAll(tableName);
...
async function ecommerceExpansion() {
  try {
    ...
    const db = await dDB.indexedDB.init("EcommerceDB", 1, [
      "users",
      "products",
      "orders",
    ]);

    console.log("📋 Tabel e-commerce awal:", db.listStores());

    // Fase 1: Tambah fitur kategori dan review
    console.log("\n🔄 Fase 1: Menambah Categories & Reviews");
    await db.addStores(["categories", "reviews"]);
    ...
    console.log("✅ Fase 1 selesai. Tabel:", db.listStores());

    // Fase 2: Tambah fitur inventory dan shipping
    console.log("\n🔄 Fase 2: Menambah Inventory & Shipping");
    await db.addStores(["inventory", "shipping"]);
    ...
    console.log("✅ Fase 2 selesai. Tabel:", db.listStores());

    // Fase 3: Tambah fitur marketing dan analytics
    console.log("\n🔄 Fase 3: Menambah Marketing & Analytics");
    await db.addStores(["coupons", "analytics", "wishlists"]);
    ...
    console.log("✅ Fase 3 selesai. Tabel:", db.listStores());

    // Fase 4: Tambah fitur advanced
    console.log("\n🔄 Fase 4: Menambah Advanced Features");
    await db.addStores(["notifications", "settings", "logs", "cache"]);
    ...
    await db.set("settings", { id: "currency", key: "currency", value: "USD" });
    await db.set("logs", {
      id: "log_1",
    ...
    console.log("✅ Fase 4 selesai. Tabel final:", db.listStores());

    // Summary
    console.log("\n📊 === E-COMMERCE EXPANSION SUMMARY ===");
    console.log(`🎯 Total tabel: ${db.listStores().length}`);
    console.log("📋 Semua tabel:", db.listStores().join(", "));
 ...
 *    await db.addStores(["categories", "reviews", "settings"]);
 *
 * 2. AUTO MODE - PALING FLEKSIBEL
 *    const db = await dDB.indexedDB.init("MyDB", 1); // Tanpa parameter
 *    await db.set("new_table", { id: "1", data: "value" }); // Tabel dibuat otomatis
 *
 * 3. SATU PER SATU
 *    await db.addStores("categories");
 *    await db.addStores("reviews");
