...
/** Instance NexaTables agar di-destroy saat navigasi ulang ke About */
let _aboutTableInstances = [];
...
export async function about(page, route) {
  route.register(page, async (routeName, container, routeMeta = {
    ...
    _aboutTableInstances.forEach((t) => {
      try {
        t.destroy();
      } catch (_) {
        /* ignore */
...
// IndexedDB: wajib objek + settings.storage + key (ID baris di bucketsStore). Bukan .buckets('demo') saja — itu hanya { id:'demo' } dan default-nya mode database.
    ...
    container.innerHTML = `
        ...
        <h2 style="margin-top:1.5rem">Data dari <code>NXUI.Storage</code></h2>
        <p class="text-muted" style="font-size:0.9rem">Tabel interaktif (cari, sort kolom, paginasi) — <code>NexaTables</code> + <code>table.css</code>.</p>
        <div id="nx-about-table-storage"></div>
        <h2 style="margin-top:1.5rem">Model query <code>news</code></h2>
        <div id="nx-about-table-model"></div>
      `;

    const t1 = await NXUI.NexaTables.fromStorageResponse(
      "#nx-about-table-storage",
      sStorage,
      {
        caption: "Storage — example().news (array di res.data)",
        pageSize: 6,
      }
    );
    _aboutTableInstances.push(t1);

    const t2 = await NXUI.NexaTables.fromStorageResponse(
      "#nx-about-table-model",
      complexQuery,
      {
        caption: "Model(table) — .model('news').select('*').get()",
        pageSize: 6,
      }
    );
    _aboutTableInstances.push(t2);
  });
}
