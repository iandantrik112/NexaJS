 ...
 */
export async function updateDataformcrossJoin(dynamicId, dataform,variable) {
  ...
  if (dataformStor.where && dataformStor.where.length > 0) {
    ...
    let idField = variable.replace("-", "."); // default
    if (dataformStor.fields && dataformStor.fields.length > 0) {
      // Cari field dengan fieldKey variable atau yang mengandung variable
      const idFieldObj = dataformStor.fields.find(
        (field) => field.fieldKey === variable || field.fieldKey.includes(variable)
      );
      if (idFieldObj) {
  ...
  if (
    ...
    !dataformStor.fieldAliasList.includes(variable)
  ) {
    dataformStor.fieldAliasList.push(variable);
  }
  ...
  try {
     ...
     if (responseData) {
       ...
       const contentHTML = Object.entries(responseData)
         .filter(([key, value]) => key !== variable) // Filter out 'id' field
         .map(([key, value]) => {
