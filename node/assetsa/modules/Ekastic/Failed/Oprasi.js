...
console.log('label:', sqlQuery);
// Hasil: SELECT petani.nama AS nama, petani.nik AS nik, petani.tempat_lahir AS tempat_lahir, petani.tanggal_lahir AS tanggal_lahir FROM petani LIMIT 5 OFFSET 0


// Ekstrak metadata SQL untuk autocomplete
const sqlMetadata = extractSQLMetadata(storage);
console.log('SQL Metadata:', sqlMetadata);
console.log('SQL Tables:', sqlMetadata.tables);
console.log('SQL Columns:', sqlMetadata.columns);
console.log('SQL Full Aliases:', sqlMetadata.fullAliases);
console.log('SQL Table Columns Map:', sqlMetadata.tableColumnsMap);
  ...
  setTimeout(async () => {
      ...
      const editorConfig = {
        ...
        onSave:async function(Code) {  
          console.log('💾 Saving SQL query...');
          console.log('📝 SQL Code:', Code);
          ...
          if (newAliases.length > 0) {
            console.log('🆕 New fields detected:', newAliases);
            addNewFieldsToStorage(newAliases, storage, updatedApplications);
          } else {
            console.log('ℹ️ No new fields detected, all fields already exist in storage');
          }
          
          // Update storage.applications dengan hasil parse
          storage.applications = updatedApplications;
          console.log('✅ storage.applications updated');
          
          // Update tabel property di form berdasarkan field yang ada di SELECT clause
          updateTabelPropertyFromSelect(storage);
          
          // Log final storage state summary
          console.log('📦 Final Storage State Summary:');
          console.log('  - buckets.allAlias:', storage);
          await NXUI.ref.set("nexaStore", storage);

          // await NXUI.ref.mergeData(storage.store, storage.id, storage);
      ...
      console.log('Editor Config dengan SQL:', editorConfig);
...
function updateTabelPropertyFromSelect(storage) {
  if (!storage || !storage.form || !storage.applications) {
    console.log('⚠️ Cannot update tabel property: storage, form, or applications not found');
    return;
  }
  ...
  console.log('🔄 Updating tabel property based on SELECT clause...');
  console.log('📋 Fields in SELECT:', selectedFields);
  console.log('📋 Total form fields:', Object.keys(storage.form).length);
  ...
  Object.keys(storage.form).forEach(fieldName => {
    ...
    const currentTabelValue = storage.form[fieldName].tabel;
    
    // Update tabel property - selalu update untuk memastikan konsistensi
    storage.form[fieldName].tabel = isInSelect;
    
    // Log jika ada perubahan atau untuk tracking
    if (currentTabelValue !== isInSelect) {
      updatedCount++;
      console.log(`  ${isInSelect ? '✅' : '❌'} Updated ${fieldName}: tabel ${currentTabelValue} → ${isInSelect}`);
    } else {
      console.log(`  ${isInSelect ? '✅' : '❌'} ${fieldName}: tabel = ${isInSelect} (unchanged)`);
    }
  ...
  console.log(`  - tabel: true = ${trueCount} fields (in SELECT)`);
  console.log(`  - tabel: false = ${falseCount} fields (not in SELECT)`);
}
...
function addNewFieldsToStorage(newAliases, storage, updatedApplications = null) {
  if (!newAliases || !Array.isArray(newAliases) || newAliases.length === 0) {
    console.log('⚠️ No new aliases to add');
    return;
  }

  if (!storage || !storage.buckets) {
    console.log('⚠️ Storage or buckets not found');
    return;
  }

  console.log('🔄 Starting to add new fields to storage...');
  console.log('📋 New aliases to add:', newAliases);
  ...
  console.log(`🔑 Base KeyIndex: ${baseKeyIndex}, Last KeyIndex: ${lastKeyIndex}`);
  ...
  newAliases.forEach((alias, index) => {
    ...
    if (!storage.buckets.allAlias.includes(fullAlias)) {
      ...
      console.log(`  ✅ Added to buckets.allAlias: ${fullAlias}`);
    }
    ...
    if (!storage.buckets.failed.includes(failedKey)) {
      ...
      console.log(`  ✅ Added to buckets.failed: ${failedKey}`);
    }
    ...
    if (!storage.buckets.variables.includes(aliasName)) {
      ...
      console.log(`  ✅ Added to buckets.variables: ${aliasName}`);
    }
    ...
    if (!storage.buckets.variablesAlias.includes(tableColumn)) {
      ...
      console.log(`  ✅ Added to buckets.variablesAlias: ${tableColumn}`);
    }
    ...
    if (!storage.variablesOrigin.includes(tableColumn)) {
      ...
      console.log(`  ✅ Added to variablesOrigin: ${tableColumn}`);
    }
    ...
    if (!storage.variables.includes(aliasName)) {
      ...
      console.log(`  ✅ Added to variables (root): ${aliasName}`);
    }
    ...
    if (!storage.form[aliasName]) {
      ...
      console.log(`  ✅ Created form object for: ${aliasName} (key: ${newKeyIndex}, tabel: ${isInSelect})`);
    }
  });
  
  if (currentKeyIndex > lastKeyIndex) {
    console.log(`🔑 KeyIndex updated: ${lastKeyIndex} → ${currentKeyIndex} (${currentKeyIndex - lastKeyIndex} new fields)`);
  }

  // Log summary perubahan
  console.log('📊 Summary of changes:');
  console.log('  - buckets.allAlias:', changes.allAlias.length, 'items added');
  console.log('  - buckets.failed:', changes.failed.length, 'items added');
  console.log('  - buckets.variables:', changes.bucketsVariables.length, 'items added');
  console.log('  - buckets.variablesAlias:', changes.variablesAlias.length, 'items added');
  console.log('  - variablesOrigin:', changes.variablesOrigin.length, 'items added');
  console.log('  - variables (root):', changes.rootVariables.length, 'items added');
  console.log('  - form:', changes.form.length, 'objects created');
  console.log('✅ All new fields added to storage successfully!');
}
