    ...
    ${
      ...
      <td  class="text-center">
       <span class="material-symbols-outlined nx-icon-md">${getIconByType(
        ...
        <td >${item.type}</td>
        <td>${await Sdk.searchKeys(item.name)}</td>
        <td>
         <div id="${
           item.name
        ...
        <td>
                  <div class="nx-switch-grid">
                    <div class="nx-switch-item">
                      <input class="${
                        item.name
                      }" name="search" type="checkbox" id="search_${
                  item.name
                }${index}"
                       ${item.search ? "checked" : ""}
                      />
                      <label for="search_${item.name}${index}">
                        <span class="nx-switch"></span>
                      </label>
                    </div>
                  </div>
...
export async function filtersetApplicationsByFilter(Sdk, fieldsToKeep) {
  // fieldsToKeep is expected to be an array of objects, each with a "failedtabel" property in "Table.field" format
  // Returns an object mapping field name to { alias, failedtabel, key }
  ...
  try {
    ...
    checkbox.onSaveCallback(async (element) => {
      ...
      if (element?.name == "inline") {
        ...
        await renderingTabel(storage);
      } else if (element?.name == "filtering") {
        await store.upField({
          [element.class]: {
            [element.name]: element.checked,
          },
