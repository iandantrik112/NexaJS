...
function generateSQLExpression(field, operation) {
  console.log(
    `generateSQLExpression called with field: ${field}, operation: ${operation}`
  );
...
function buildComplexExpression(nested) {
  console.log("buildComplexExpression called with nested:", nested);
  ...
  if (nested.length === 1) {
    ...
    console.log("Single operation detected:", singleOperation);
    ...
    if (
      ...
      const result = generateSQLExpression(
        singleOperation.field,
        singleOperation.operation
      );
      console.log("Special operation result:", result);
      return result;
    }
  }
...
export async function saveComplexCustom(
  modalid,
  data,
  tabel,
  custom,
  ...
  try {
    ...
    const oldArithmetic =
      dataform?.[custom]?.arithmetic || dataform?.arithmetic || [];
    ...
    const complexExpression = buildComplexExpression(nested, tabel);
    ...
    const makeDir = {
      [custom]: {
        ...(dataform[custom] || {}),
        arithmetic: validItems,
      },
    };

    console.log("About to save makeDir:", makeDir);
    console.log("Current dataform[custom]:", dataform[custom]);
    console.log("Valid items to save:", validItems);

    await NXUI.ref.mergeData(tabel.store, tabel.id, makeDir);

    // Close the modal
    NXUI.nexaModal.close(modalcustom);
