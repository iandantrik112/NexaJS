 ...
 */
import { NexaDB } from "../Storage/db.js";
...
import { setConfiguration, setKeyPackages } from "./Configuration/index.js";
...
export class NexaEkastic {
  ...
  async initializeDatabase() {
    try {
      ...
      // Initialize database reference with required stores using NexaDB
      try {
        const storage = new NexaDB(1);
        await storage.initDatabase();
        await storage.install();
        NXUI.ref = storage; // Set the reference to NexaDB instance
      } catch (versionError) {
        if (
          ...
          if (actualVersion) {
            ...
            const storage = new NexaDB(actualVersion);
            await storage.initDatabase();
            await storage.install();
            NXUI.ref = storage;
          } else {
