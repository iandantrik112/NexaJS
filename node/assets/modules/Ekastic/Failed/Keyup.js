    ...
    setTimeout(async () => {
      try {
        ...
        let debounceTimer = null;
           ...
           NXUI.NexaKeyup.fromFormData(storage?.form, { 
            delay: 100, 
            carakter :100, 
...
export async function setformslug(result2, data,height) {
 ...
 const btn=result2[0] ? 'Update':'Save';
  ...
  <small class="text-muted align-right">
   <button onclick="addtags('${data?.id}');" class="nx-btn-primary custom-size-sm">${btn} Generator</button>
  </small>
...
export async function rendering(store) {
  ...
  await NXUI.NexaRender.refresh(store, setFailed, {
    containerSelector: ["#contentsetFailed"],
  });
}
