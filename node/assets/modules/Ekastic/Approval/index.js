    ...
    const wrapper = NXUI.createElement(
     ...
     <div id="renderFormNav">
         <div class="nx-card-header">
           <h3 class="bold fs-20px"> Approval</h3>  
      ...
      try {
        ...
        const template = render.Container({
          ...
          content: [
            ...
            await settingsApproval(Sdk,height),
  ...
  try {
    ...
    nexaField.onSaveCallback(
      async (variable, newValue, element, type, fieldName) => {
        ...
        await renderingFormApproval(storage);
      }
    );
...
export async function settingsApproval(Sdk,height) {
  const storage = await Sdk.storage();
  const checkedItems = await Sdk.getFields("approval");
  let tabelHtml = `
    ...
    ${
      ...
      <td  class="text-center" >
       ...
       <td> 
         <span id="${
           item.name
         }" type="text" class="editable" data-min-length="5" name="receive">${
                item.receive
              }</span>
          </td>
         <td> 
         <span id="${
           item.name
         }" type="text" class="editable" data-min-length="5" name="reject">${
                item.reject
              }</span>
          </td>
             ...
             <td class="text-center">
                  <div class="nx-switch-grid pl-20px">
                    <div class="nx-switch-item">
                      <input class="${item.name}" name="approdialog" type="checkbox" id="approdialog${index}" ${item.approdialog ? "checked" : ""}/>
                      <label for="approdialog${index}">
...
<div class="nx-alert nx-alert-primary">
      <h3>Panduan Penggunaan Fitur Approval</h3>
      <div style="line-height: 1.8;">
        <p><strong>1. Mengaktifkan Approval untuk Field</strong></p>
        <ul style="margin-left: 20px; margin-bottom: 15px;">
          <li>Pilih field yang ingin diaktifkan fitur approval dari panel <strong>"Setting Failed"</strong> di sebelah kanan</li>
          <li>Aktifkan toggle switch pada field yang diinginkan</li>
          <li>Field yang aktif akan muncul di tabel <strong>"Properti"</strong> dengan konfigurasi default</li>
        </ul>
        
        <p><strong>2. Konfigurasi Approval</strong></p>
        <ul style="margin-left: 20px; margin-bottom: 15px;">
          <li><strong>Icons:</strong> Klik icon untuk mengubah icon approval yang ditampilkan</li>
          <li><strong>Label:</strong> Klik pada teks "Label" untuk mengedit placeholder/teks label yang ditampilkan</li>
          <li><strong>Receive:</strong> Klik untuk mengedit teks tombol "Terima" (default: "Permohonan diterima")</li>
          <li><strong>Reject:</strong> Klik untuk mengedit teks tombol "Tolak" (default: "Permohonan ditolak")</li>
          <li><strong>Modal Reject:</strong> Aktifkan toggle untuk menampilkan modal konfirmasi saat tombol "Tolak" diklik</li>
        </ul>
        ...
        <ul style="margin-left: 20px;">
          <li>Gunakan <strong>Modal Reject</strong> untuk meminta konfirmasi sebelum menolak permohonan</li>
          <li>Sesuaikan teks <strong>Receive</strong> dan <strong>Reject</strong> sesuai kebutuhan aplikasi</li>
          <li>Pastikan <strong>Label</strong> diisi dengan teks yang jelas dan informatif</li>
          <li>Pilih icon yang sesuai dengan konteks approval (misalnya: check_circle, cancel, attach_file)</li>
        </ul>
      </div>
</div>
...
export async function Failed(Sdk,height) {
  ...
  if (fields && fields.length > 0) {
    for (const row of fields) {
      const checkedAttr = row.approval ? "checked" : "";
...
export async function renderingFormApproval(store) {
  await NXUI.NexaRender.refresh(store, setApproval, {
    containerSelector:"#renderingFormApproval",
  });
}
export async function setCheckbox(store) {
  try {
    ...
    checkbox.onSaveCallback(async (element) => {
      const storage = await store.storage();
      if (element.name == "approdialog") {
      console.log('label:', element);
         await store.upField({
           [element.class]: {
             approdialog:element.checked,
           },
         });
        ...
        await store.upField({
           [element.name]: {
             ...
             icons: "attach_file",
             receive: "Permohonan diterima",
             reject: "Permohonan ditolak",
             approval: element.checked,
             approdialog: false,
           },
         });
      }
