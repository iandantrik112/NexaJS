...
export async function maxQuery(storage) {
  try {
    // Cek apakah ada query yang sudah tersimpan di operasi
    if (storage.applications && storage.applications.operasi) {
      // Cari query dari operasi (ambil query pertama yang ditemukan)
      const operasiKeys = Object.keys(storage.applications.operasi);
      for (const key of operasiKeys) {
        const operasiData = storage.applications.operasi[key];
        if (operasiData && operasiData.query) {
          // Gunakan query dari operasi langsung
          return operasiData.query;
        }
      }
    }
    // Jika tidak ada query di operasi, generate dari applications
    return convertApplicationsToSQL(storage.applications);
  } catch (error) {
    return `
 ...
 */
export function updateTabelPropertyFromSelect(storage) {
 ...
 */
export function addNewFieldsToStorage(newAliases, storage, updatedApplications = null) {
 ...
 */
export function parseSQLToApplications(sqlQuery, currentApplications = {}) {
  ...
  if (selectMatch) {
    ...
    columns.forEach(column => {
      ...
      if (aliasMatch) {
        ...
        // Jangan tambahkan tabel dari SELECT clause - hanya FROM clause yang menentukan tabel
      } else if (column === '*') {
        // Handle SELECT *
        updated.alias = ['*'];
        updated.aliasNames = [];
  ...
  // Parse FROM clause
  // Cari posisi FROM dan ambil semua teks sampai sebelum WHERE/GROUP BY/ORDER BY/HAVING/LIMIT/OFFSET
  const fromIndex = normalizedQuery.search(/\sFROM\s/i);
  if (fromIndex !== -1) {
    const afterFrom = normalizedQuery.substring(fromIndex + 5).trim(); // 5 = length of " FROM"
    
    // Cari akhir FROM clause (sebelum WHERE, GROUP BY, ORDER BY, HAVING, LIMIT, OFFSET)
    const fromEndMatch = afterFrom.match(/^(.+?)(?:\s+(?:WHERE|GROUP\s+BY|ORDER\s+BY|HAVING|LIMIT|OFFSET|$))/i);
    let fromClause = fromEndMatch ? fromEndMatch[1].trim() : afterFrom.trim();
    
    // Jika ada JOIN, ambil hanya bagian sebelum JOIN untuk ekstrak tabel yang dipisahkan koma
    const beforeJoinMatch = fromClause.match(/^(.+?)(?:\s+(?:INNER|LEFT|RIGHT|FULL)\s+JOIN)/i);
    if (beforeJoinMatch) {
      fromClause = beforeJoinMatch[1].trim();
    }
    
    // Split berdasarkan koma untuk mendapatkan semua tabel
    const tables = fromClause.split(',').map(t => t.trim()).filter(t => t.length > 0);
    // Hanya ambil tabel dari FROM clause, jangan gabungkan dengan yang sudah ada
    updated.tabelName = [...new Set(tables)];
    
    // Ekstrak tabel dari JOIN clause juga
    const joinMatch = normalizedQuery.match(/(?:INNER|LEFT|RIGHT|FULL)\s+JOIN\s+(\w+)/i);
    if (joinMatch) {
      const joinTable = joinMatch[1].trim();
      if (!updated.tabelName.includes(joinTable)) {
        updated.tabelName.push(joinTable);
      }
    }
  }
  ...
  // Pastikan tidak ada duplikat di tabelName sebelum return
  updated.tabelName = [...new Set(updated.tabelName)];
...
export function saveOprasi(Code,storage,setType) {
       const updatedApplications = parseSQLToApplications(Code, storage.applications);
          ...
          if (tableName) {
            ...
            storage.query=setType
            storage.applications.operasi[tableName] = {
              ...operasiData,
              "type":setType // Type untuk operasi.demo adalah "operasi"
            };
