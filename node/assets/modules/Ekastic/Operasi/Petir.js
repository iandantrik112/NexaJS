...
import { 
  maxQuery,
  extractSQLMetadata,
  parseSQLToApplications,
  updateTabelPropertyFromSelect,
  saveOprasi } from "./maxQuery.js";
export async function opPetir(data) {
  try {
  ...
  setTimeout(async () => {
      ...
      const editorConfig = {
        ...
        height: '100%',
        title: "Petir",
        save: 'petirbtnSave',   // ID tombol save (akan di-setup otomatis)
        copy: 'petirbtnCopy',   // ID tombol copy (akan di-setup otomatis)
        ...
        onSave:async function(Code) {  
          const finalStorage = saveOprasi(Code, storage, 'petir');
          finalStorage.applications.operasi[finalStorage.tableName].query = Code;
          finalStorage.applications.order=false;
          finalStorage.applications.where=false;
          finalStorage.applications.group=false;
      ...
      const editor = new NXUI.NexaCmirror('codeEditorPetir', editorConfig);
 ...
 <div class="nx-card NexaCmirror-editorContainer" style="opacity: 0; visibility: hidden;">
  <div class="nx-card-header">
    ...
    <div class="NexaCmirror-editorToolbar">
      ...
      <button id="petirbtnCopy" class="NexaCmirror-btn-copy" title="Copy">
        <span class="material-symbols-outlined">content_copy</span>
      </button>
      <button id="petirbtnSave" class="NexaCmirror-btn-save" title="Save">
  ...
  <div>
<textarea id="codeEditorPetir">${addQue}</textarea>
  </div>
  </div>
  <div class="nx-card-footer">
    <div class="nx-alert nx-alert-primary">
      <h5 class="bold mb-2"><span class="material-symbols-outlined">bolt</span> Petir - Ultra Advanced SQL Editor</h5>
      <div class="mb-2">
        <strong>Fitur Utama:</strong>
        <ul class="mb-0 mt-1" style="padding-left: 20px;">
          <li><strong>Editor SQL Ultra-Advanced:</strong> Editor SQL untuk query super kompleks dengan autocomplete lengkap</li>
          <li><strong>Nested Queries:</strong> Support untuk nested queries dan subquery yang sangat kompleks</li>
          <li><strong>Advanced Operations:</strong> CTE, Window Functions, dan advanced database operations</li>
          <li><strong>Query Management:</strong> Simpan dan kelola query kompleks dengan mudah</li>
        </ul>
      </div>
      <div class="mb-2">
        <strong>Cara Penggunaan:</strong>
        <ul class="mb-0 mt-1" style="padding-left: 20px;">
          <li>Edit query SQL langsung di editor untuk operasi database yang kompleks</li>
          <li>Gunakan <kbd>Ctrl+Shift+F</kbd> atau tombol Format untuk merapikan kode</li>
          <li>Klik <strong>Save</strong> untuk menyimpan query ke storage</li>
          <li>Query akan tersimpan dengan type 'petir' untuk operasi advanced</li>
        </ul>
      </div>
      <div class="mb-0">
        <strong>Note:</strong> Petir adalah versi yang lebih advanced dari Complex, dirancang untuk power users dan query yang sangat kompleks dengan dukungan operasi database tingkat lanjut.
      </div>
    </div>
  </div>
</div>
