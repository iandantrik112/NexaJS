import { NexaInstall } from "./install.js";
import { EkasticTabel } from "../../Tabel/EkasticTabel.js";
import { EkasticContent } from "../../Application/object/index.js";
export async function setPackage(key, data) {
  try {
    ...
    if (tabel?.packages) {
      ...
      if (content=='object') {
         const packageInstance = new EkasticContent(tabel);
         return await packageInstance.init(tabel,height);
      } else {
        const packageInstance = new EkasticTabel(key);
...
nx.selectDataType = async function (type, id) {
  try {
    // Use dynamic import to load NexaInstall from install.js
    // This ensures the class is available when called from onclick handler
    const { NexaInstall } = await import("./install.js");
    const installInstance = new NexaInstall();
    return await installInstance.showInstallModal(id, type);
  } catch (error) {
    console.error("Error in selectDataType:", error);
  }
};
