...
import { updateDatacallback } from "./callback.js";
...
export class SearchField {
  ...
  async displaySuggestions(query, inputId, suggestionsId, hasilId) {
    ...
    suggestions.forEach((suggestion) => {
      ...
      suggestionItem.addEventListener("click", async () => {
        ...
        if (inputEl) inputEl.value = suggestion.id;
        ...
        if (this.model == "return") {
          try {
            ...
            if (firstData && firstData.id) {
              ...
              // Selalu jalankan callback jika ada callbackFunction, tidak perlu element
              if (this.callbackFunction) {
                const dataformStor = this.dataform.formSDK;
                const callbackResult = await updateDatacallback(
                  dataformStor,
                  firstData.id,
                  this.dataform
                );

                // Store callback result and call callback function
                this.callbackResults = callbackResult;
                this.callbackFunction(callbackResult);
              }
            }
            ...
            if (firstData &&  firstData.label) {
              ...
              if (firstData.label) {
                if (this.dataform.type == "join") {
                  await updateDataformcrossJoin(firstData.label, this.dataform,this.options.variable);
  ...
  init(inputId, suggestionsId, hasilId) {
    ...
    // Store callback results and callback function
    ...
    // Return object with callback function instead of promise
    return {
      onCallback: (callback) => {
