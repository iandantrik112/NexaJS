...
export async function Form(tabel) {
  try {
    ...
    whereConditions.forEach((item, index) => {
      ...
      templateField += `
        <li class="nx-list-item" id="wherekey${index}">
          ...
          <a class="pull-right" onclick="removeWhereCondition(${index});" href="javascript:void(0);">
            <span class="material-symbols-outlined nx-icon-sm">delete_sweep</span>
          </a>
        </li>
      `;
    ...
    nx.clearWhere = async function () {
      const makeDir = { where: { conditions: [], alias: "" } };
    ...
    nx.removeWhereCondition = async function (index) {
