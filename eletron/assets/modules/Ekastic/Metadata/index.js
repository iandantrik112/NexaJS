...
const height = dimensi.height("#report-container", 180, 'vh');
console.log(height);
    const checkedAccess =storage.access === "private" && storage.access ? "checked" : "";
    ...
    setTimeout(async () => {
      try {
        ...
        const template = render.Container({
          container: "#nxdrop",
          content: [await propertiesTable(Sdk,height), await Failed(Sdk,height)],
        });
      ...
      const nexaScroll = new NXUI.NexaScroll({
        storageKey: 'nexaScrollPositions2',
        debounceDelay: 100,
        autoInit: true
      });
...
export async function propertiesTable(Sdk,height) {
  const storage = await Sdk.storage();
    ...
    ${
      ...
      <td>${item.type}</td>
        ...
        <td>
         <div id="${
           item.name
         }" type="text" class="editable" data-min-length="2" name="placeholder">${
                  item.placeholder
                }</div>
...
console.log(height);
  ...
  <small class="text-muted align-right">
   <button onclick="updateBucketSingel('${storage?.id}');" class="nx-btn-primary custom-size-sm">Update Bucket</button>
  </small>
...
nx.updateBucketSingel = async function (id) {
...
export async function Failed(Sdk,height) {
  ...
  const itemPromises = fields.map(async (row, index) => {
...
export async function renderingTabel(store) {
  ...
  await setApplications(store.id);
}
