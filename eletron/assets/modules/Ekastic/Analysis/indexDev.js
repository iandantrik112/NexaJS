...
const dataTabel = await NXUI.Storage().models("Office").directAn
  ...
  try {
    ...
    // Jika storage null, buat object kosong
      ...
      const updatedaction = {
       ...
       await renderingAnalysis(storage)
  } catch (error) {
    throw error;
  }
};
...
nx.settingsObjAction = async function (objkey, id) {
  console.log('label:', objkey, id);
    ...
    const nameCode= generateUniqueName(modalID);
    NXUI.modalHTML({
      elementById: modalID,
      styleClass: "w-600px",
      minimize: true,
      label: action.label,
      onclick:false,
      content: `await newLayar(dataform, action,objkey,modalID,nameCode)`,
    });
...
nx.deleteObjAction = async function (objkey, id) {
  try {
    ...
    // Refresh layar untuk update tampilan
     await renderingAnalysis(finalStorage);
  ...
  try {
    ...
    nexaField.onSaveCallback(
      async (variable, newValue, element, type, fieldName) => {
        ...
        // Refresh tampilan
        await renderingAnalysis(await Sdk.storage());
      }
    );
...
export async function renderingAnalysis(store) {
