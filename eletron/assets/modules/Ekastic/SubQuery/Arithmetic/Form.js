...
import { setComplexCustom } from "./complex.js";
import { saveComplexCustom } from "./saveComplex.js";
...
export async function ArithmeticFormCustom(tabel, modalID, custom) {
  try {
    let template = "";
    ...
    const arithmeticList = Array.isArray(tabel[custom]?.arithmetic)
      ? tabel[custom].arithmetic
      : [];
      ...
      <div class="nx-container">
        ...
        <div class="nx-col-12">
          <div class="form-nexa-group">
            <button onclick="nx.setComplexCustomJoinTabelCustom('${
              tabel.id
            }','${
      tabel.store
    }','${custom}');" type="button" class="nx-btn-white full-width">Add Complex Arithmetic</button>
          </div>
        </div>
...
nx.setComplexCustomJoinTabelCustom = async function (id, tabel, custom) {
  try {
    ...
    NXUI.modalHTML({
      ...
      onclick: false,
      content: await setComplexCustom(dataform, modalID.custom),
    });
    ...
    NXUI.modalHTML({
      ...
      onclick: {
        title: "Save Nested",
        cancel: "Cancel",
        send: "saveArithmeticNestedCustom", // ✅ Use namespaced function name
      },
      content: await nested(dataform, modalID, custom),
    });
...
nx.saveArithmeticNestedCustom = async function (modalid, data, tabel) {
  let custom = NXUI.addcustom;
  let modalcustom = NXUI.closcustom;
  await saveComplexCustom(modalid, data, tabel, custom, modalcustom);
};
