export class NexaDirectory {
  constructor(containerId, instance, contentContainerId = null) {
    this.nexaUI = NexaUI();
    this.container = document.getElementById(containerId);
    this.contentContainerId = contentContainerId || "report-container"; // Default to report-container if not specified
    // Generate unique storage keys based on containerId to avoid conflicts between instances
    const containerIdSuffix = containerId.replace(/[^a-zA-Z0-9]/g, "_");
    this.storageKey = `nexaTree_expandedItems_${containerIdSuffix}`;
    this.activeMenuKey = `nexaTree_activeMenu_${containerIdSuffix}`;
    ...
    NexaDirectory.setActiveInstance(this);
  }
  ...
  async showContentActionInfo(itemData, priority = "normal") {
    const reportContainer = document.getElementById(this.contentContainerId);
  ...
  // Helper untuk menampilkan informasi item di content container
  displayItemInfo(itemData) {
    const contentContainer = document.getElementById(this.contentContainerId);
    if (!contentContainer) {
      console.warn(`Content container not found: ${this.contentContainerId}`);
      return;
    }

    // Build HTML untuk menampilkan informasi item (menggunakan HTML sederhana yang pasti ter-render)
    let html = `
      <div style="padding: 20px; background: #f8f9fa; border-radius: 8px; border: 1px solid #dee2e6;">
        <h3 style="margin-top: 0; margin-bottom: 20px; color: #333; display: flex; align-items: center; gap: 10px;">
          ...
          <span>${this.escapeHtml(itemData.label || "Item Information")}</span>
        </h3>
    ...
    if (itemData.key) {
      html += `
        <div style="display: flex; gap: 10px;">
          <strong style="min-width: 100px; color: #666;">Key:</strong>
          <code style="background: #e9ecef; padding: 2px 6px; border-radius: 3px;">${this.escapeHtml(itemData.key)}</code>
        </div>
      `;
    }

    // Display href if available
    if (itemData.href) {
      html += `
        <div style="display: flex; gap: 10px; align-items: center;">
          <strong style="min-width: 100px; color: #666;">URL:</strong>
          <a href="${this.escapeHtml(itemData.href)}" target="_blank" style="color: #007bff; text-decoration: none;">${this.escapeHtml(itemData.href)}</a>
        </div>
      `;
    }
    ...
    if (itemData.action) {
      html += `
        <div style="display: flex; gap: 10px;">
          <strong style="min-width: 100px; color: #666;">Action:</strong>
          <code style="background: #e9ecef; padding: 2px 6px; border-radius: 3px;">${this.escapeHtml(itemData.action)}</code>
        </div>
      `;
    }
    ...
    if (itemData.label) {
      html += `
        <div style="display: flex; gap: 10px;">
          <strong style="min-width: 100px; color: #666;">Label:</strong>
          <span>${this.escapeHtml(itemData.label)}</span>
        </div>
      `;
    }
    ...
    if (itemData.icon) {
      html += `
        <div style="display: flex; gap: 10px; align-items: center;">
          <strong style="min-width: 100px; color: #666;">Icon:</strong>
          <i data-feather="${this.escapeHtml(itemData.icon)}"></i>
          <span>${this.escapeHtml(itemData.icon)}</span>
        </div>
      `;
    }

    // Display other properties
    const excludedProps = ['key', 'label', 'href', 'action', 'icon', 'submenu', 'children'];
    Object.keys(itemData).forEach(key => {
      if (!excludedProps.includes(key) && itemData[key] !== null && itemData[key] !== undefined) {
        const value = typeof itemData[key] === 'object' 
          ? JSON.stringify(itemData[key], null, 2) 
          : String(itemData[key]);
        html += `
          <div style="display: flex; gap: 10px;">
            <strong style="min-width: 100px; color: #666;">${this.escapeHtml(key)}:</strong>
            <pre style="margin: 0; background: #f1f3f5; padding: 8px; border-radius: 4px; overflow-x: auto; font-size: 12px;">${this.escapeHtml(value)}</pre>
          </div>
        `;
      }
    });
    ...
    console.log(`Item info displayed in container: ${this.contentContainerId}`, itemData);
  ...
  async restoreActiveMenu() {
    ...
    try {
      ...
      if (menuElement) {
        ...
        let parent = menuElement.closest(".nx-tree-submenu");
        while (parent) {
          const parentId = parent.id;
          const parentKey = parentId.replace("-submenu", "").replace("tree-item-", "");
          const parentLabel = this.container.querySelector(
            `[data-processed-key="${parentKey}"]`
          );
          
          if (parentLabel) {
            const expandIcon = parentLabel.querySelector(".nx-tree-expand-icon");
            const submenu = document.getElementById(parentId);
            
            if (expandIcon && !expandIcon.classList.contains("nx-tree-expanded")) {
              expandIcon.classList.add("nx-tree-expanded");
              this.expandedItems.add(parentKey);
            }
            if (submenu && !submenu.classList.contains("nx-tree-expanded")) {
              submenu.classList.add("nx-tree-expanded");
            }
          }
          
          // Move up to next parent
          parent = parent.parentElement?.closest(".nx-tree-submenu");
        }
        
        // Save expanded items after expanding parents
        ...
        if (!this.isModalAction(itemData.action)) {
          // Check if it's a viewPage action that should display item info
          const isViewPageAction = itemData.action && itemData.action.startsWith("viewPage:");
          const hasInstanceMethod = this.instance && typeof this.instance[itemData.action] === "function";
          const isContentAction = this.isContentAction(itemData.action);
          
          // For viewPage actions without instance method, display item info directly
          if (isViewPageAction && !hasInstanceMethod && !isContentAction) {
            this.displayItemInfo(itemData);
          } else {
            // For other content actions, use showContentActionInfo
            await this.showContentActionInfo(itemData, "restore");
            
            // Fallback: if content is empty after restore, show item info
            setTimeout(() => {
              const contentContainer = document.getElementById(this.contentContainerId);
              if (contentContainer && (!contentContainer.innerHTML || contentContainer.innerHTML.trim() === "")) {
                this.displayItemInfo(itemData);
              }
            }, 500);
          }
  ...
  generateIcon(iconName, hasChildren = false, isExpanded = false) {
    // NexaTree hanya menerima dan menampilkan icon yang sudah siap dari NexaStore
    let featherIcon = iconName || "help"; // fallback default
    
    // Auto-detect folder vs file: jika icon tidak diset dan punya children, gunakan folder icon
    if (!iconName && hasChildren) {
      // Use folder-open when expanded, folder when collapsed
      featherIcon = isExpanded ? "folder" : "folder"; // Both use "folder" (Feather doesn't have folder-open)
      // Alternative: can use "chevron-down" indicator or keep "folder" for both
    } else if (!iconName && !hasChildren) {
      featherIcon = "file";
    }
  ...
  generateTreeItem(item, level = 0) {
    ...
    let html = `
      ...
      "&apos;"
            ...
            ${this.generateIcon(item.icon, hasChildren, isExpanded)}
  ...
  async handleItemClick(event, itemId, hasChildren) {
    ...
    if (action) {
      ...
      if (action && itemData) {
        ...
        if (this.isModalAction(action)) {
          ...
          // Check if action is viewPage (from href) - these are simple file links
          const isViewPageAction = action.startsWith("viewPage:");
          const hasInstanceMethod = this.instance && typeof this.instance[action] === "function";
          const isContentAction = this.isContentAction(action);
          
          // If it's viewPage action and no instance method or not a recognized content action, show item info directly
          if (isViewPageAction && !hasInstanceMethod && !isContentAction) {
            this.saveActiveMenu(itemData, itemId);
            console.log("Displaying item info for viewPage action:", action, itemData);
            this.displayItemInfo(itemData);
            return {
              success: true,
              action: action,
              itemData: itemData,
              message: "File information displayed",
            };
          }
          ...
          try {
            ...
            // Check if content was actually updated after a short delay
            setTimeout(() => {
              const contentContainer = document.getElementById(this.contentContainerId);
              if (contentContainer) {
                const hasContent = contentContainer.innerHTML && contentContainer.innerHTML.trim() !== "";
                if (!hasContent && !isContentAction) {
                  // No content was rendered, show item info as fallback
                  console.log("No content rendered, showing item info fallback");
                  this.displayItemInfo(itemData);
                }
              }
            }, 500);
            ...
            // Fallback: show item info if action fails
            this.displayItemInfo(itemData);
          }
        }
      }
    } else if (!hasChildren) {
      // Handle file click without action: show item information
      const itemData = parseItemData();
      ...
      // Display item information in content container
      this.displayItemInfo(itemData);

      // Return result for file without action
      return {
        success: true,
        expanded: false,
        itemId: itemId,
    ...
    if (action && this.isModalAction(action)) {
      const itemData = parseItemData();
    ...
    // Return default result untuk items dengan action yang sudah diproses
    if (action) {
      const itemData = parseItemData();
      return {
        success: true,
  ...
  static setupNexaStoreConnection(nexaStoreInstance) {
    ...
    window.nexaStoreMenuRefreshCallback = (newMenuStructure) => {
      // Cari NexaDirectory instance yang aktif
      const activeTreeInstance = NexaDirectory.getActiveInstance();
      if (activeTreeInstance && activeTreeInstance.container) {
        activeTreeInstance.render(newMenuStructure);
        return;
      }
      ...
      for (const containerId of commonContainers) {
        ...
        if (container) {
          try {
            const tempInstance = new NexaDirectory(containerId);
      ...
      document.dispatchEvent(
        new CustomEvent("nexaTreeRefreshFailed", {
  ...
  static setActiveInstance(instance) {
    NexaDirectory._activeInstance = instance;
  }

  static getActiveInstance() {
    return NexaDirectory._activeInstance || null;
  }

  static clearActiveInstance() {
    NexaDirectory._activeInstance = null;
  }
  ...
  destroy() {
    ...
    if (NexaDirectory.getActiveInstance() === this) {
      NexaDirectory.clearActiveInstance();
    }
  ...
  static setupNexaStoreConnection(nexaStoreInstance) {
    ...
    if (window.nexaStoreConnectionManager) {
      const activeTreeInstance =
        NexaDirectory.getActiveInstance() || window.nexaTreeInstance;
    ...
    import("../System/init.js")
      ...
      .catch((error) => {
        ...
        NexaDirectory.legacySetupConnection(nexaStoreInstance);
      });
  }
